...
public class ShaderChooser {
	...
	public static String getUsableShaderFileName(int type) {
		if(OptionHandler.getProperty(EngineOptions.SHADERUSECUSTOM_KEY, OptionHandler.ENGINE_OPTION_ID).equals("1")) {
			...
			if(OptionHandler.getProperty(GraphicOptions.USEVIEWMATRIX_KEY, OptionHandler.GRAPHIC_OPTION_ID).equals("1")) {
				if(type == VERTEX_TYPE) {
			...
			if(OptionHandler.getProperty(GraphicOptions.USEPROJECTIONMARTRIX_KEY, OptionHandler.GRAPHIC_OPTION_ID).equals("1")) {
				if(type == VERTEX_TYPE) {
